...
func init() {
	android.RegisterModuleType("palladium_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = palladiumExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = palladiumExpandVariables(ctx, dep_file)
	...
	cmd := palladiumExpandVariables(ctx, String(g.properties.Cmd))
