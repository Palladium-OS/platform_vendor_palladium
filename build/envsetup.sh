...
cat <<EOF
Additional PalladiumOS functions:
...
- githubremote:    Add git remote for PalladiumOS Github.
...
function palladiumremote()
{
    ...
    local PALLADIUM_USER=$(git config --get review.review.palladiumos.com.username)
    if [ -z "$PALLADIUM_USER" ]
    then
        git remote add palladium ssh://review.palladiumos.com:29418/$PFX$PROJECT
    else
        git remote add palladium ssh://$PALLADIUM_USER@review.palladiumos.com:29418/$PFX$PROJECT
    fi
...
function palladiumgerrit() {
    ...
    local user=`git config --get review.review.palladiumos.com.username`
...
function palladiumrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "PalladiumOS Gerrit Rebase Usage: "
    ...
    git fetch "http://review.palladiumos.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
