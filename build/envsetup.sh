...
- palladiumgerrit:   A Git wrapper that fetches/pushes patch from/to Palladium-OS Gerrit Review.
- palladiumrebase:   Rebase a Gerrit change and push it again.
- palladiumremote:   Add git remote for Palladium-OS Gerrit Review.
...
function palladiumremote()
{
    ...
    then
        ...
        local PFX="Palladium-OS/"
    else
        local PROJECT=$REMOTE
    fi

    local PALLADIUM_USER=$(git config --get review.review..username)
    if [ -z "$PALLADIUM_USER" ]
    then
        git remote add palladium ssh://review.:29418/$PFX$PROJECT
    else
        git remote add palladium ssh://$PALLADIUM_USER@review.:29418/$PFX$PROJECT
    fi
...
function palladiumrebase() {
    ...
    git fetch "http://review./p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                palladium|otapackage|bacon|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
